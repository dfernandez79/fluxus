...
ExtractPages.prototype = {
        ...
        if (!err) {
          try {
            self._parseFiles(stdout).forEach(function (file, index) {
              var dest = path.join(outputDir, self._outputPrefix + '-' + (index + 1) + '@2x.png');
  ...
  start: function () {
    this._emitter.emit('watch', {action: this});

    // watch is better in terms of performance, but for some reason it doesn't detect Illustrator file changes :(
    fs.watchFile(this._inputFile, function (curr, prev) {
      if (curr.mtime > prev.mtime) {
        this._emitter.emit('change', {action: this});
        this.run();
      }
    }.bind(this));
  },
  ...
  stop: function () {
    fs.unwatchFile(this._inputFile);
  },
